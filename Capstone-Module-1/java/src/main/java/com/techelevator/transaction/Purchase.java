...
public class Purchase extends Menu {
	...
	private List<String> consumeNoises = new ArrayList<String>();
	...
	public void selectItem(VendoMatic800 vendingMachine, FeedMoney feedMoney) {
		...
		while (isInvalid) {
			...
			try (FileWriter fw = new FileWriter("log.txt", true);
				...
				vendingMachine.getVendingMachine().get(userInput).getName(), userInput, balance,
				(balance - vendingMachine.getVendingMachine().get(userInput).getPrice()));
